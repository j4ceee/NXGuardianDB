...
// check if table games is empty
useDB();
$stmt = $PDO->prepare("SELECT 1 FROM games");
$stmt->execute();
...
?>
        ...
        if ($PDO != null) {
            if (checkDBExists()) {
                ...
                if (file_exists('./db/bk.sql') && !$tableExists) {
                    echo '<a href="./util/restore_db.php" class="db-restore">
                            <svg class="status-symbol" width="100%" height="100%" viewBox="0 0 1200 1200" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
